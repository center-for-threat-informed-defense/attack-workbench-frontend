...
export class Technique extends StixObject {
  ...
  public serialize(keepModified?: string): any {
    ...
    if (this.domains.includes('enterprise-attack')) {
      ...
      if (this.tactics.includes('defense-evasion'))
        rep.stix.x_mitre_defense_bypassed = this.defense_bypassed.map((x) => x.trim());
      ...
      if (this.domains.includes('enterprise-attack')) {
        if (this.capec_ids.length) {
          for (const id of this.capec_ids) {
            const temp = {};
            temp['url'] =
              'https://capec.mitre.org/data/definitions/' + id.trim().split('-')[1] + '.html';
          ...
          for (let i = 0; i < sdo.external_references.length; i++) {
            if (
              ...
              if (typeof (sdo.external_references[i].external_id === 'string')) {
                ...
                else if (sdo.external_references[i].source_name == 'NIST Mobile Threat Catalogue')
  ...
  public validate(restAPIService: RestApiConnectorService): Observable<ValidationData> {
    return this.base_validate(restAPIService).pipe(
      map((result) => {
        ...
        // check CAPEC IDs
        const malformed_capec = this.capec_ids.filter((capec) => !/^CAPEC-\d+$/.test(capec));
        ...
        // check MTC IDs
        const mtc_regex = new RegExp(`^(${Object.keys(this.mtcUrlMap).join('|')})-\\d+$`);
  ...
  private updateParentRelationship(restApiService: RestApiConnectorService): Observable<any> {
    if (this.is_subtechnique && this.parentTechnique) {
      ...
      return restApiService
        .getRelatedTo({ sourceRef: this.stixID, relationshipType: 'subtechnique-of' })
        .pipe(
          switchMap((r) => {
  ...
  private syncTacticsWithParentOrSubs(restApiService: RestApiConnectorService): Observable<any> {
            ...
            if (r.data.length > 0) {
              ...
              return restApiService.getTechnique(relationship.target_ref, null, 'latest').pipe(
                switchMap((parentData) => {
                  ...
                  if (
                    parent &&
                    !this.killChainPhasesSynced(parent.kill_chain_phases, this.kill_chain_phases)
                  ) {
                    // this sub-technique's tactics are not synced with its parent
                    const parentTactics = parent.kill_chain_phases.map((kcp) => [
                      kcp.phase_name,
    ...
    // case: parent technique, need to check if parent has sub-techniques
      ...
      return restApiService
        .getRelatedTo({ targetRef: this.stixID, relationshipType: 'subtechnique-of' })
        .pipe(
          switchMap((r) => {
                ...
                return restApiService.getTechnique(subRelationship.source_ref, null, 'latest').pipe(
                  switchMap((subData) => {
                    ...
                    if (
                      ...
                      const parentTactics = this.kill_chain_phases.map((kcp) => {
                        const killChainName = Object.keys(this.killChainMap).find(
                          (key) => this.killChainMap[key] === kcp.kill_chain_name,
                        );
