...
export class DatepickerPropertyComponent {
  ...
  public get formatHint() {
    return DATE_FORMATS.parse.dateInput;
  }
  public get date() {
    // get date in view display format (i.e. January 2022)
  ...
  public get current() {
    const d = this.object?.[this.config.field] || '';
    return moment.utc(d).format('MMMM YYYY');
  }

  public get previous() {
    const d = this.config.object[1]?.[this.config.field] || '';
    return moment.utc(d).format('MMMM YYYY');
  }
