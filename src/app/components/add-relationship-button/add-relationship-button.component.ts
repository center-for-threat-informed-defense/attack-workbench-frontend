...
export class AddRelationshipButtonComponent implements OnInit {
  ...
  public createRelationship() {
    ...
    const sourceType = this.config.sourceType ? this.config.sourceType : null;
    ...
    var zip_subscription = initializer.subscribe({
      ...
      complete: () => {
        if (zip_subscription) zip_subscription.unsubscribe();
      }, //for some reason zip_subscription doesn't exist if using set_source_object or set_target_object
    });
  }
}
