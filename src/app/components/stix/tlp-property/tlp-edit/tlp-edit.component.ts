...
export class TlpEditComponent implements OnInit {
  ...
  private initializeSelectAndSetCurrentSelection() {
    ...
    for (const key in this.tlpMarkingDefinitionsMap) {
  ...
  public updateTlpMarking() {
    ...
    const current_selection = this.initializeSelectAndSetCurrentSelection();
    ...
    const subscription = prompt.afterClosed().subscribe({
      next: (result) => {
        if (result && this.select.selected) {
          ...
          for (const selection of this.select.selected) {
